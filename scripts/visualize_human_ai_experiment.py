...
def visualize_trajectory_1D(data) -> None:
    ...
    y_truth = np.array(observer(tf.convert_to_tensor(x_linspace)).observations)
...
def visualize_trajectory_2D(data) -> None:
    """Visualizes the end result (approximation, sample data, etc)
    ...
    def draw_results(slider_input: float) -> int:
        ...
        if b < n:
            ...
            axs["ax_3d"].scatter(
                map_arg_max[0], map_arg_max[1], map_max, color="red", label="MAP"
            )
...
def visualize_moo(results):
    ...
    x_linspaces = [
        np.linspace(x_min, x_max, grid_n) for x_min, x_max in zip(*problem.bounds)
    ]
    X_mesh = np.meshgrid(*x_linspaces, indexing="ij")
    O_x = np.array(
        observer(tf.convert_to_tensor(np.stack(X_mesh, axis=-1))).observations
    )

    o_lims = [
        (O_x[..., o].min().item(), O_x[..., o].max().item()) for o in range(num_objs)
    ]
    o_linspaces = [np.linspace(o_min, o_max, grid_n) for o_min, o_max in o_lims]
    O_mesh = np.meshgrid(*o_linspaces, indexing="xy")
    ...
    if "map" in results["results"]:
        map_x = np.array(results["results"]["map"]["arg_max"])
        map_obj = np.array(results["results"]["map"]["obj"])
        map_u = np.array(results["results"]["map"]["max"])
    else:
        map_x = np.zeros_like(queries)
        map_u = np.zeros_like(utilities)
        map_obj = np.zeros_like(objectives)
